...
import time
import pickle
from src.e3_diffusion.qm9.utils import prepare_context, compute_mean_mad
...
def main():
    if args.resume is not None:
        flow_state_dict = torch.load(
            join(args.resume, "generative_model_ema.npy"), map_location=device
        )  # "flow.npy"
        # optim_state_dict = torch.load( we dont have optimizer
        #     join(args.resume, "optim.npy"), map_location=device
        # )
    ...
    print(args.ema_decay)
    if args.ema_decay > 0:
        if args.resume is not None:
            # When resuming, the loaded model IS the EMA model
            print("my model is ema")
            model_ema = model
        else:
            # Only create fresh EMA model when starting from scratch
