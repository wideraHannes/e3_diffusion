...
from src.e3_diffusion.qm9 import losses
...
def train_epoch(
    ...
    for i, data in enumerate(loader):
        ...
        if (
            ...
            vis.visualize(
                f"src/e3_diffusion/outputs/{args.exp_name}/epoch_{epoch}_{i}",
                dataset_info=dataset_info,
                wandb=wandb,
            )
            vis.visualize_chain(
                f"src/e3_diffusion/outputs/{args.exp_name}/epoch_{epoch}_{i}/chain/",
                dataset_info,
                wandb=wandb,
            )
            if len(args.conditioning) > 0:
...
def save_and_sample_chain(
    ...
    vis.save_xyz_file(
        f"src/e3_diffusion/outputs/{args.exp_name}/epoch_{epoch}_{batch_id}/chain/",
        one_hot,
        charges,
        x,
        dataset_info,
...
def sample_different_sizes_and_save(
    ...
    for counter in range(int(n_samples / batch_size)):
        ...
        vis.save_xyz_file(
            f"src/e3_diffusion/outputs/{args.exp_name}/epoch_{epoch}_{batch_id}/",
            one_hot,
            charges,
            x,
            dataset_info,
...
def save_and_sample_conditional(
    args, device, model, prop_dist, dataset_info, epoch=0, id_from=0
):
