...
class TransformerNN(nn.Module):
    def __init__(
        ...
        self.initial_mlp = MLP(
            in_node_nf, hidden_nf, hidden_initial, initial_mlp_layers, skip=1, bias=True
        )
        ...
        for _ in n_layers:
            self.decoder_layers.append(
                nn.TransformerEncoderLayer(
                    hidden_nf, n_heads, dim_feedforward, dropout=0
                )
            )

        self.final_mlp = MLP(
            hidden_nf, out_node_nf, hidden_final, final_mlp_layers, skip=1, bias=True
        )
    ...
    def forward(self, h, x, edges, edge_attr=None, node_mask=None, edge_mask=None):
        ...
        for i in range(len(self.decoder_layers)):
            ...
            if self.use_bn and type(x).__name__ != "TransformerEncoderLayer":
                out = self.bn_layers[i](out.transpose(1, 2)).transpose(
                    1, 2
                )  # bs, n, hidden
            x = (
                out + x
                if self.res and type(x).__name__ != "TransformerEncoderLayer"
                else out
            )
        return x
    ...
    class SetDecoder(nn.Module):
        ...
        def forward(self, x, latent):
            ...
            latent: batch_size, channels2."""
            x = F.relu(
                self.initial_mlp(x, latent[:, self.cosine_channels :].unsqueeze(1))
            )
            for i in range(len(self.decoder_layers)):
                out = F.relu(self.decoder_layers[i](x))  # [bs, n, d]
                if self.use_bn and type(x).__name__ != "TransformerEncoderLayer":
...
class MLP(nn.Module):
    def __init__(
        self, dim_in: int, dim_out: int, width: int, nb_layers: int, skip=1, bias=True
    ):
        """
        Args:
